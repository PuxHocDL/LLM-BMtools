...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Judge model name from config.yaml (defaults to google/gemma-4-31B-it)",
    )
    ...
    parser.add_argument("--strategy", type=str, default=None,
                        choices=["s1_prompt", "s2_compress", "s3_cot", "s4_twostage", "s5_fewshot", "s6_context", "s7_selfdebug"],
                        help="Enhancement strategy to use (overrides --enhanced)")
    args = parser.parse_args()
    if args.judge is None:
        args.judge = "google/gemma-4-31B-it"
